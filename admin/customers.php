...
{
  $result = $conn->query("SELECT customer_id, profile_image, id_number, first_name, last_name, contact_number, email, customer_type, status FROM customers ORDER BY customer_id DESC");
  $customers = [];
...
function saveCustomerProfileImage($base64Data)
{
  ...
  $uploadDirFs = __DIR__ . '/../uploads/customers/';
  if (!is_dir($uploadDirFs)) {
    mkdir($uploadDirFs, 0755, true);
  }

  $filename = 'customer_' . uniqid() . '_' . time() . '.' . $ext;
  file_put_contents($uploadDirFs . $filename, $data);

  return 'uploads/customers/' . $filename;
}

function deleteCustomerProfileImage($dbRelativePath)
{
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
  ...
  if ($action === 'edit_customer') {
    ...
    $stmt = $conn->prepare("SELECT profile_image, password FROM customers WHERE customer_id = ? LIMIT 1");
    ...
    if ($removeImage) {
      deleteCustomerProfileImage($profileImagePath);
    ...
    $stmt->bind_param("sssssssssi", $profileImagePath, $idNumberToSave, $firstName, $lastName, $contact, $emailToSave, $passwordToSave, $custType, $status, $customerId);
    $ok = $stmt->execute();
    $stmt->close();
  ...
  if ($action === 'delete_customer') {
    ...
    $stmt = $conn->prepare("SELECT profile_image FROM customers WHERE customer_id = ? LIMIT 1");
    $stmt->bind_param("i", $customerId);
    $stmt->execute();
    $existing = $stmt->get_result()->fetch_assoc();
    $stmt->close();
    ...
    if ($ok && $existing) {
      deleteCustomerProfileImage($existing['profile_image']);
    }
  ...
  if ($action === 'get_customers') {
    echo json_encode([
      'success'   => true,
      'customers' => fetchCustomersData($conn),
    ]);
    $conn->close();
    exit;
  }
...
$initialCustomers = fetchCustomersData($conn);
$conn->close();
?>
<!doctype html>
<html lang="en">
...
<body class="bg-white">
  <div class="flex flex-col h-screen">
    ...
    <aside id="sidebar">
      ...
      <nav class="flex-1 overflow-y-auto py-3 px-3 space-y-1">
        ...
        <a href="./customers.php" class="sidebar-link active flex items-center gap-3 px-3 py-2.5 text-emerald-700 bg-emerald-50 border border-emerald-100 font-semibold transition-colors" style="border-radius:6px">
  ...
  <div
    ...
    <div class="modal-overlay absolute inset-0" id="closeCustomerOverlay"></div>
    <div
      ...
      <div
        ...
        <h2 class="font-bold text-gray-800 text-sm" id="customerModalTitle">
          Add Customer
        </h2>
        <button
      ...
      <div class="p-4 space-y-3">
        <div id="profileImageSection" class="hidden flex-col items-center gap-2">
          <div class="relative">
            <div
              id="profilePreview"
        ...
        <div>
          <label
            class="block text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Customer Type</label>
          <div class="grid grid-cols-2 gap-2">
            <label
              class="flex items-center gap-2 p-2.5 border border-gray-200 cursor-pointer hover:border-emerald-500 transition-all has-[:checked]:border-emerald-500 has-[:checked]:bg-emerald-50/40"
              style="border-radius: 3px">
              <input
                type="radio"
                name="customerType"
              ...
              <span class="text-xs font-medium text-gray-700">Student</span>
            </label>
            <label
              class="flex items-center gap-2 p-2.5 border border-gray-200 cursor-pointer hover:border-emerald-500 transition-all has-[:checked]:border-emerald-500 has-[:checked]:bg-emerald-50/40"
              style="border-radius: 3px">
              <input
                ...
                value="faculty"
                class="accent-emerald-600 shrink-0" />
              <span class="text-xs font-medium text-gray-700">Faculty</span>
            </label>
            <label
              class="flex items-center gap-2 p-2.5 border border-gray-200 cursor-pointer hover:border-emerald-500 transition-all has-[:checked]:border-emerald-500 has-[:checked]:bg-emerald-50/40"
              style="border-radius: 3px">
        ...
        <div id="idNumberFieldWrap">
          <label
            class="block text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1.5">ID Number</label>
          <input
            type="text"
            id="fieldIdNumber"
  ...
  <div
    ...
    <div class="modal-overlay absolute inset-0" id="closeDeleteCustomerOverlay"></div>
    <div
      ...
      <div class="flex items-center gap-2.5">
        ...
        <div>
          <p class="text-sm font-bold text-gray-800">Delete Customer</p>
          <p class="text-[10px] text-gray-400 mt-0.5" id="deleteCustomerName"></p>
        </div>
      </div>
      <p class="text-xs text-gray-500">
        This customer record will be permanently removed. This cannot be
        undone.
      </p>
      <div class="flex gap-2 pt-1">
        <button
          id="cancelDeleteCustomerBtn"
  ...
  <script>
    ...
    async function refreshCustomers() {
      const res = await postAction("get_customers");
      if (res.success) {
    ...
    function renderList() {
      ...
      let filtered = customers.filter((c) => {
        ...
        const matchType = currentType === "all" || c.customer_type === currentType;
          ...
          return `
            <div class="px-4 py-3 flex items-center gap-3">
              ...
              <div class="flex-1 min-w-0">
                <p class="text-xs font-semibold text-gray-800 truncate">${escapeHtml(c.first_name)} ${escapeHtml(c.last_name)}</p>
                <div class="flex items-center gap-1.5 mt-1 flex-wrap">
                  ${typeBadge(c.customer_type)}
                  ${statusBadge(c.status)}
                </div>
                <p class="text-[11px] text-gray-400 truncate mt-1">${displayOrDash(c.contact_number)} &middot; ${displayOrDash(c.email)}</p>
              </div>
              <button class="p-1 hover:bg-gray-100 transition-colors edit-customer-btn shrink-0" data-id="${c.customer_id}" title="Edit customer" style="border-radius:3px">
              ...
              <button class="p-1 hover:bg-red-50 transition-colors delete-customer-btn shrink-0" data-id="${c.customer_id}" title="Delete customer" style="border-radius:3px">
    ...
    function openEditCustomerModal(id) {
      ...
      currentProfileImage = c.profile_image || null;
      removeImageFlag = false;
      ...
      document.getElementById("fieldIdNumber").value = c.id_number || "";
      document.getElementById("fieldContact").value = c.contact_number;
      ...
      document.querySelector(`input[name='customerType'][value='${c.customer_type}']`).checked = true;
      document.querySelector(`input[name='customerStatus'][value='${c.status}']`).checked = true;
      toggleIdNumberField();
      document.getElementById("customerFormError").classList.add("hidden");
      document.getElementById("customerFormError").classList.remove("flex");
      ...
      updateProfilePreview(c.profile_image || null, getInitials(c.first_name, c.last_name));
      resetPasswordMeter();
      document.getElementById("customerModal").classList.remove("hidden");
      document.body.style.overflow = "hidden";
    }
    ...
    async function saveCustomer() {
      ...
      const status = document.querySelector("input[name='customerStatus']:checked").value;

      const errEl = document.getElementById("customerFormError");
      const errMsg = document.getElementById("customerFormErrorMsg");
      ...
      const res = isEditing ?
        await postAction("edit_customer", {
          customer_id: editingCustomerId,
          ...payload
        }) :
        await postAction("add_customer", payload);
    ...
    window.addEventListener("load", function() {
      ...
      document.querySelectorAll("input[name='customerType']").forEach((radio) => {
        radio.addEventListener("change", toggleIdNumberField);
      });
      document.getElementById("closeCustomerModalBtn").addEventListener("click", closeCustomerModal);
      document.getElementById("closeCustomerOverlay").addEventListener("click", closeCustomerModal);
      ...
      document.getElementById("fieldFirstName").addEventListener("input", () => {
        if (editingCustomerId && !currentProfileImage)
      ...
      document.getElementById("closeDeleteCustomerOverlay").addEventListener("click", closeDeleteCustomerModal);
      ...
      document.getElementById("confirmDeleteCustomerBtn").addEventListener("click", async () => {
        const res = await postAction("delete_customer", {
          customer_id: deletingCustomerId
        });
