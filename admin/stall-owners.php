...
{
  $result = $conn->query("SELECT owner_id, profile_image, first_name, last_name, contact_number, email, status FROM stall_owners ORDER BY owner_id DESC");
  $owners = [];
...
function saveOwnerProfileImage($base64Data)
{
  ...
  $uploadDirFs = __DIR__ . '/../uploads/stall/';
  if (!is_dir($uploadDirFs)) {
    mkdir($uploadDirFs, 0755, true);
  }

  $filename = 'owner_' . uniqid() . '_' . time() . '.' . $ext;
  file_put_contents($uploadDirFs . $filename, $data);

  return 'uploads/stall/' . $filename;
}

function deleteOwnerProfileImage($dbRelativePath)
{
...
function ownerEmailTakenByOther($conn, $email, $excludeOwnerId = 0)
{
  ...
  $stmt = $conn->prepare("SELECT owner_id FROM stall_owners WHERE email = ? AND owner_id != ? LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
  ...
  if ($action === 'add_owner') {
    ...
    if (ownerEmailTakenByOther($conn, $email)) {
    ...
    $stmt = $conn->prepare("INSERT INTO stall_owners (profile_image, first_name, last_name, contact_number, email, password, status) VALUES (?, ?, ?, ?, ?, ?, ?)");
    ...
    echo json_encode($ok
      ? ['success' => true]
      : ['success' => false, 'message' => 'Failed to add owner.']);
    $conn->close();
    exit;
  }
  ...
  if ($action === 'edit_owner') {
    ...
    if (ownerEmailTakenByOther($conn, $email, $ownerId)) {
      echo json_encode(['success' => false, 'message' => 'This email address is already in use.']);
      $conn->close();
      exit;
    }
    ...
    $stmt = $conn->prepare("SELECT profile_image, password FROM stall_owners WHERE owner_id = ? LIMIT 1");
    ...
    if ($removeImage) {
      deleteOwnerProfileImage($profileImagePath);
      $profileImagePath = null;
    ...
    $stmt->bind_param("sssssssi", $profileImagePath, $firstName, $lastName, $contact, $email, $passwordToSave, $status, $ownerId);
    $ok = $stmt->execute();
    $stmt->close();

    echo json_encode($ok
      ? ['success' => true]
      : ['success' => false, 'message' => 'Failed to update owner.']);
    $conn->close();
    exit;
  }
  ...
  if ($action === 'delete_owner') {
    ...
    $stmt = $conn->prepare("SELECT profile_image FROM stall_owners WHERE owner_id = ? LIMIT 1");
    $stmt->bind_param("i", $ownerId);
    $stmt->execute();
    $existing = $stmt->get_result()->fetch_assoc();
    $stmt->close();

    $stmt = $conn->prepare("DELETE FROM stall_owners WHERE owner_id = ?");
    $stmt->bind_param("i", $ownerId);
    $ok = $stmt->execute();
    $stmt->close();

    if ($ok && $existing) {
      deleteOwnerProfileImage($existing['profile_image']);
    }
...
<head>
  ...
  <title>Admin - Stall Owners</title>
...
<body class="bg-white">
  <div class="flex flex-col h-screen">
      ...
      <div class="max-w-5xl mx-auto px-4 py-2 flex items-center justify-between gap-3">
        <div class="flex items-center gap-2 min-w-0">
          ...
          <nav class="flex items-center gap-1.5 text-xs text-gray-500 min-w-0" aria-label="Breadcrumb">
            ...
            <span class="text-emerald-600 font-medium truncate">Stall Owners</span>
          </nav>
        </div>
        <button
          id="addOwnerBtn"
    ...
    <aside id="sidebar">
      ...
      <nav class="flex-1 overflow-y-auto py-3 px-3 space-y-1">
        ...
        <a href="./stall-owners.php" class="sidebar-link active flex items-center gap-3 px-3 py-2.5 text-emerald-700 bg-emerald-50 border border-emerald-100 font-semibold transition-colors" style="border-radius:6px">
        ...
        <div class="rounded-md bg-white border border-gray-200 shadow-sm overflow-hidden">
          <div class="p-4 border-b border-gray-100 flex items-center justify-between">
            <p class="text-xs font-bold text-gray-700">
              All Owners
              <span class="text-gray-400 font-normal" id="ownerCount"></span>
            </p>
          </div>
          <div id="ownerList" class="divide-y divide-gray-100"></div>
          <div id="emptyState" class="hidden py-12 text-center">
            ...
            <p class="text-sm font-semibold text-gray-500">No owners found</p>
  ...
  <div
    ...
    <div class="modal-overlay absolute inset-0" id="closeOwnerOverlay"></div>
    <div
      ...
      <div
        ...
        <h2 class="font-bold text-gray-800 text-sm" id="ownerModalTitle">
          Add Owner
        </h2>
        <button
          id="closeOwnerModalBtn"
  ...
  <div
    ...
    <div class="modal-overlay absolute inset-0" id="closeDeleteOwnerOverlay"></div>
    <div
      ...
      <div class="flex items-center gap-2.5">
        ...
        <div>
          <p class="text-sm font-bold text-gray-800">Delete Owner</p>
          <p class="text-[10px] text-gray-400 mt-0.5" id="deleteOwnerName"></p>
        </div>
      </div>
      <p class="text-xs text-gray-500">
        This owner record will be permanently removed. This cannot be
        undone.
      </p>
      <div class="flex gap-2 pt-1">
        <button
          id="cancelDeleteOwnerBtn"
          class="flex-1 py-2 border border-gray-200 text-gray-700 text-xs font-semibold hover:bg-gray-50"
          style="border-radius: 3px">
          Cancel
        </button>
        <button
          id="confirmDeleteOwnerBtn"
  ...
  <script>
    let owners = <?php echo json_encode($initialOwners); ?>;
    ...
    async function refreshOwners() {
      const res = await postAction("get_owners");
      if (res.success) {
        owners = res.owners;
        renderList();
      }
    ...
    function avatarHtml(owner) {
      if (owner.profile_image) {
        return `<img src="${escapeHtml(owner.profile_image)}" class="w-9 h-9 rounded-full object-cover shrink-0" />`;
      }
      return `<div class="w-9 h-9 bg-gradient-to-br from-emerald-500 to-emerald-700 flex items-center justify-center text-white text-[10px] font-bold shrink-0 rounded-full">${getInitials(owner.first_name, owner.last_name)}</div>`;
    }
    ...
    function renderList() {
      ...
      document.getElementById("ownerCount").textContent = `(${filtered.length})`;
          ...
          return `
            <div class="px-4 py-3 flex items-center gap-3">
              ...
              <div class="flex-1 min-w-0">
                <p class="text-xs font-semibold text-gray-800 truncate">${escapeHtml(o.first_name)} ${escapeHtml(o.last_name)}</p>
                <div class="flex items-center gap-1.5 mt-1 flex-wrap">
                  ${statusBadge(o.status)}
                </div>
                <p class="text-[11px] text-gray-400 truncate mt-1">${displayOrDash(o.contact_number)} &middot; ${displayOrDash(o.email)}</p>
              </div>
              <button class="p-1 hover:bg-gray-100 transition-colors edit-owner-btn shrink-0" data-id="${o.owner_id}" title="Edit owner" style="border-radius:3px">
              ...
              <button class="p-1 hover:bg-red-50 transition-colors delete-owner-btn shrink-0" data-id="${o.owner_id}" title="Delete owner" style="border-radius:3px">
      ...
      container.querySelectorAll(".edit-owner-btn").forEach((btn) => {
        btn.addEventListener("click", () => openEditOwnerModal(parseInt(btn.dataset.id)));
      });
      container.querySelectorAll(".delete-owner-btn").forEach((btn) => {
        btn.addEventListener("click", () => openDeleteOwnerModal(parseInt(btn.dataset.id)));
      });
    }
    ...
    function openAddOwnerModal() {
      ...
      document.querySelector("input[name='ownerStatus'][value='active']").checked = true;
      document.getElementById("ownerFormError").classList.add("hidden");
      document.getElementById("ownerFormError").classList.remove("flex");
    ...
    function openEditOwnerModal(id) {
      const o = owners.find((x) => x.owner_id === id);
      if (!o) return;
      editingOwnerId = id;
      currentProfileImage = o.profile_image || null;
      removeImageFlag = false;
      document.getElementById("ownerModalTitle").textContent = "Edit Owner";
      document.getElementById("fieldFirstName").value = o.first_name;
      document.getElementById("fieldLastName").value = o.last_name;
      document.getElementById("fieldContact").value = o.contact_number;
      document.getElementById("fieldEmail").value = o.email;
      document.getElementById("fieldPassword").value = "";
      ...
      document.querySelector(`input[name='ownerStatus'][value='${o.status}']`).checked = true;
      document.getElementById("ownerFormError").classList.add("hidden");
      document.getElementById("ownerFormError").classList.remove("flex");
      document.getElementById("profileImageInput").value = "";
      updateProfilePreview(o.profile_image || null, getInitials(o.first_name, o.last_name));
      resetPasswordMeter();
      document.getElementById("ownerModal").classList.remove("hidden");
      document.body.style.overflow = "hidden";
    }
    ...
    function openDeleteOwnerModal(id) {
      const o = owners.find((x) => x.owner_id === id);
      if (!o) return;
      deletingOwnerId = id;
      document.getElementById("deleteOwnerName").textContent = `${o.first_name} ${o.last_name}`;
      document.getElementById("deleteOwnerModal").classList.remove("hidden");
      document.body.style.overflow = "hidden";
    }
    ...
    async function saveOwner() {
      ...
      const status = document.querySelector("input[name='ownerStatus']:checked").value;

      const errEl = document.getElementById("ownerFormError");
      const errMsg = document.getElementById("ownerFormErrorMsg");

      const isEditing = !!editingOwnerId;
      ...
      const res = isEditing ?
        await postAction("edit_owner", {
          owner_id: editingOwnerId,
          ...payload
        }) :
        await postAction("add_owner", payload);
      ...
      closeOwnerModal();
      await refreshOwners();
      showToast(isEditing ? "Owner updated successfully" : "Owner added successfully");
    }
    ...
    window.addEventListener("load", function() {
      ...
      document.getElementById("closeOwnerOverlay").addEventListener("click", closeOwnerModal);
      document.getElementById("cancelOwnerBtn").addEventListener("click", closeOwnerModal);
      ...
      document.getElementById("closeDeleteOwnerOverlay").addEventListener("click", closeDeleteOwnerModal);
      document.getElementById("cancelDeleteOwnerBtn").addEventListener("click", closeDeleteOwnerModal);
      document.getElementById("confirmDeleteOwnerBtn").addEventListener("click", async () => {
        const res = await postAction("delete_owner", {
          owner_id: deletingOwnerId
        });
        ...
        if (res.success) {
          ...
          showToast(res.message || "Failed to delete owner", "warning");
        }
      });
