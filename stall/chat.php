...
$statusCheckStmt = $conn->prepare("SELECT status, first_name, last_name FROM stall_owners WHERE owner_id = ? LIMIT 1");
...
$ownerName = trim($statusCheckRow['first_name'] . ' ' . $statusCheckRow['last_name']);
...
{
  if ($type === 'customer') {
    $stmt = $conn->prepare("SELECT first_name, last_name, profile_image, contact_number, customer_type FROM customers WHERE customer_id = ? LIMIT 1");
    ...
    return [
      ...
      'subLabel' => ucfirst($row['customer_type']),
...
function findOrCreateConversation($conn, $ownerId, $otherType, $otherId)
{
  $stmt = $conn->prepare("
    SELECT conversation_id FROM conversations
    WHERE (participant_a_type = 'stall_owner' AND participant_a_id = ? AND participant_b_type = ? AND participant_b_id = ?)
       OR (participant_b_type = 'stall_owner' AND participant_b_id = ? AND participant_a_type = ? AND participant_a_id = ?)
    LIMIT 1
  ");
  $stmt->bind_param("isiisi", $ownerId, $otherType, $otherId, $ownerId, $otherType, $otherId);
  $stmt->execute();
  $row = $stmt->get_result()->fetch_assoc();
  $stmt->close();
  ...
  $stmt = $conn->prepare("INSERT INTO conversations (participant_a_type, participant_a_id, participant_b_type, participant_b_id) VALUES ('stall_owner', ?, ?, ?)");
  $stmt->bind_param("isi", $ownerId, $otherType, $otherId);
  $stmt->execute();
...
function verifyOwnerInConversation($conn, $conversationId, $ownerId)
{
  $stmt = $conn->prepare("
    SELECT conversation_id FROM conversations
    WHERE conversation_id = ?
      AND (
        (participant_a_type = 'stall_owner' AND participant_a_id = ?)
        OR (participant_b_type = 'stall_owner' AND participant_b_id = ?)
      )
    LIMIT 1
  ");
  $stmt->bind_param("iii", $conversationId, $ownerId, $ownerId);
  $stmt->execute();
  $row = $stmt->get_result()->fetch_assoc();
  $stmt->close();
  return !!$row;
...
function getOtherParticipant($conn, $conversationId, $ownerId)
{
  $stmt = $conn->prepare("SELECT participant_a_type, participant_a_id, participant_b_type, participant_b_id FROM conversations WHERE conversation_id = ? LIMIT 1");
  ...
  if ($row['participant_a_type'] === 'stall_owner' && (int) $row['participant_a_id'] === $ownerId) {
    return ['type' => $row['participant_b_type'], 'id' => (int) $row['participant_b_id']];
  }
  return ['type' => $row['participant_a_type'], 'id' => (int) $row['participant_a_id']];
}

function fetchConversationsForOwner($conn, $ownerId)
{
  $stmt = $conn->prepare("
    SELECT conversation_id, participant_a_type, participant_a_id, participant_b_type, participant_b_id
    FROM conversations
    WHERE (participant_a_type = 'stall_owner' AND participant_a_id = ?)
       OR (participant_b_type = 'stall_owner' AND participant_b_id = ?)
  ");
  ...
  while ($row = $result->fetch_assoc()) {
    ...
    $unreadStmt = $conn->prepare("SELECT COUNT(*) AS cnt FROM messages WHERE conversation_id = ? AND sender_type != 'stall_owner' AND is_read = 0");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
  ...
  if ($action === 'get_conversations') {
    echo json_encode(['success' => true, 'conversations' => fetchConversationsForOwner($conn, $ownerId)]);
    $conn->close();
    exit;
  }
  ...
  if ($action === 'open_conversation') {
    ...
    if (!in_array($withType, ['customer', 'admin'], true) || $withId <= 0) {
    ...
    $conversationId = findOrCreateConversation($conn, $ownerId, $withType, $withId);
  ...
  if ($action === 'get_messages') {
    ...
    if (!verifyOwnerInConversation($conn, $conversationId, $ownerId)) {
      echo json_encode(['success' => false, 'message' => 'Conversation not found.']);
      $conn->close();
      exit;
    }

    $markStmt = $conn->prepare("UPDATE messages SET is_read = 1 WHERE conversation_id = ? AND sender_type != 'stall_owner' AND is_read = 0");
    ...
    while ($row = $result->fetch_assoc()) {
      $messages[] = [
        ...
        'sender' => $row['sender_type'] === 'stall_owner' ? 'me' : 'them',
  ...
  if ($action === 'poll_messages') {
    ...
    while ($row = $result->fetch_assoc()) {
      if ($row['sender_type'] !== 'stall_owner') {
  ...
  if ($action === 'send_message') {
    ...
    $stmt = $conn->prepare("INSERT INTO messages (conversation_id, sender_type, sender_id, message_text) VALUES (?, 'stall_owner', ?, ?)");
    $stmt->bind_param("iis", $conversationId, $ownerId, $text);
    $ok = $stmt->execute();
    $newMessageId = $conn->insert_id;
    $stmt->close();

    if ($ok) {
      $otherParticipant = getOtherParticipant($conn, $conversationId, $ownerId);
      if ($otherParticipant) {
        createNotification(
          $conn,
...
if (in_array($openWithType, ['customer', 'admin'], true) && $openWithId > 0) {
  $info = fetchOtherPartyInfo($conn, $openWithType, $openWithId);
  if ($info) {
    $initialConversationId = findOrCreateConversation($conn, $ownerId, $openWithType, $openWithId);
  }
}
    ...
    function renderConversationList() {
      ...
      if (filtered.length === 0) {
        ...
        if (conversations.length === 0) {
          emptyEl.querySelector("p.text-sm").textContent = "No conversations yet";
          emptyEl.querySelector("p.text-xs").textContent = "Messages from customers will appear here";
        } else {
          emptyEl.querySelector("p.text-sm").textContent = "No conversations found";
          emptyEl.querySelector("p.text-xs").textContent = "Try a different search";
        }
        return;
      }
