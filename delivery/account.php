...
function fetchStaffProfile($conn, $staffId)
{
  $stmt = $conn->prepare("SELECT profile_image, first_name, last_name, contact_number, email, status, created_at FROM delivery_staff WHERE staff_id = ? LIMIT 1");
...
function handleStaffProfileImageUpload($file)
{
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
  ...
  if ($action === 'edit_profile') {
    ...
    $stmt = $conn->prepare("UPDATE delivery_staff SET first_name = ?, last_name = ?, contact_number = ?, email = ? WHERE staff_id = ?");
    $stmt->bind_param("ssssi", $firstName, $lastName, $contact, $email, $staffId);
    $ok = $stmt->execute();
    $stmt->close();
    ...
    if (isset($_FILES['profile_image_file']) && $_FILES['profile_image_file']['error'] !== UPLOAD_ERR_NO_FILE) {
      $imagePath = handleStaffProfileImageUpload($_FILES['profile_image_file']);
      if ($imagePath) {
        $imgStmt = $conn->prepare("UPDATE delivery_staff SET profile_image = ? WHERE staff_id = ?");
        $imgStmt->bind_param("si", $imagePath, $staffId);
        $imgStmt->execute();
        $imgStmt->close();
      ...
      $imgStmt = $conn->prepare("UPDATE delivery_staff SET profile_image = NULL WHERE staff_id = ?");
      $imgStmt->bind_param("i", $staffId);
      $imgStmt->execute();
      $imgStmt->close();
      $newProfileImageRaw = null;
      $hasNewImage = true;
    ...
    if ($hasNewImage) {
      ...
      $currentProfile = fetchStaffProfile($conn, $staffId);
  ...
  if ($action === 'change_password') {
    ...
    $stmt = $conn->prepare("SELECT password FROM delivery_staff WHERE staff_id = ? LIMIT 1");
    $stmt->bind_param("i", $staffId);
    $stmt->execute();
    $row = $stmt->get_result()->fetch_assoc();
    $stmt->close();
    ...
    $updateStmt = $conn->prepare("UPDATE delivery_staff SET password = ? WHERE staff_id = ?");
    $updateStmt->bind_param("si", $newPw, $staffId);
...
$initialProfile = fetchStaffProfile($conn, $staffId);
$conn->close();
...
<head>
  ...
  <title>Delivery Staff - My Account</title>
...
<body class="bg-white">
      ...
      <div class="max-w-5xl mx-auto px-4 pt-3 pb-4 space-y-3">
        ...
        <div class="grid grid-cols-2 gap-2">
          <div class="rounded-md bg-white border border-gray-200 shadow-sm p-3 text-center">
            <p id="statDeliveries" class="text-base font-bold text-emerald-600"></p>
            <p class="text-[10px] text-gray-400 mt-0.5">Total Deliveries</p>
          </div>
          <div class="rounded-md bg-white border border-gray-200 shadow-sm p-3 text-center">
      ...
      <div class="p-4 space-y-3">
        ...
        <div>
          ...
          <div class="relative">
            ...
            <input type="password" id="fieldNewPassword" placeholder="At least 8 characters" class="w-full pl-9 pr-9 py-2.5 bg-white border border-gray-200 text-xs text-gray-900 placeholder-gray-400 focus:outline-none focus:border-emerald-600 rounded-[3px]" />
            <button
              type="button"
  ...
  <script>
    ...
    function renderProfile() {
      const initials = getInitials(deliveryAccount.firstName, deliveryAccount.lastName);
      document.getElementById("profileAvatar").innerHTML = deliveryAccount.profileImage ?
        `<img src="${escapeHtml(deliveryAccount.profileImage)}" class="w-full h-full object-cover" style="border-radius:50%" />` :
        initials;
      document.getElementById("profileName").textContent =
        deliveryAccount.firstName + " " + deliveryAccount.lastName;
      document.getElementById("profileSubtext").textContent = deliveryAccount.email;

      document.getElementById("statDeliveries").textContent = deliveryAccount.totalDeliveries;
      document.getElementById("statMemberSince").textContent = deliveryAccount.memberSince;
    ...
    function openEditProfileModal() {
      document.getElementById("fieldFirstName").value = deliveryAccount.firstName;
      document.getElementById("fieldLastName").value = deliveryAccount.lastName;
      document.getElementById("fieldContact").value = deliveryAccount.contactNumber;
      document.getElementById("fieldEmail").value = deliveryAccount.email;
      ...
      updateEditProfilePreview(
        deliveryAccount.profileImage || null,
        getInitials(deliveryAccount.firstName, deliveryAccount.lastName),
      );
      initialEditProfileState = {
        firstName: deliveryAccount.firstName,
        lastName: deliveryAccount.lastName,
        contact: deliveryAccount.contactNumber,
    ...
    async function saveEditProfile() {
      ...
      deliveryAccount.firstName = firstName;
      deliveryAccount.lastName = lastName;
      deliveryAccount.contactNumber = contact;
      deliveryAccount.email = email;
      if (res.profile_image !== undefined) {
        deliveryAccount.profileImage = res.profile_image;
      }
