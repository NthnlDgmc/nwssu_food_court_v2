...
function fetchDashboardStats($conn, $staffId)
{
  ...
  $stmt = $conn->prepare("SELECT COALESCE(SUM(total_delivery_fee), 0) AS total FROM orders WHERE staff_id = ? AND status = 'delivered' AND DATE(proof_captured_at) = CURDATE()");
  $stmt->bind_param("s", $staffId);
  $stmt->execute();
  $stats['todaysEarnings'] = (float) $stmt->get_result()->fetch_assoc()['total'];
  $stmt->close();
