...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
  ...
  if ($action === 'cancel_order') {
    ...
    $stmt->bind_param("ssi", $reason, $orderId, $customerId);
    $ok = $stmt->execute();
    $stmt->close();
  ...
  if ($action === 'update_location') {
    ...
    $stmt->bind_param("ssi", $location, $orderId, $customerId);
    $ok = $stmt->execute();
    $stmt->close();
    ...
    <script>
      ...
      function bindModalEvents() {
        ...
        if (saveBtn) {
          saveBtn.addEventListener("click", async () => {
            const orderIdRaw = saveBtn.getAttribute("data-id");
      ...
      function bindEvents() {
        ...
        document.querySelectorAll(".cancel-btn").forEach((btn) => {
          btn.addEventListener("click", () =>
            openCancelModal(btn.getAttribute("data-id")),
          );
        });
        document.querySelectorAll(".confirm-receipt-btn").forEach((btn) => {
          btn.addEventListener("click", () => {
            if (btn.disabled) return;
            openConfirmReceiptModal(btn.getAttribute("data-id"));
          });
        });
        document.querySelectorAll(".report-issue-btn").forEach((btn) => {
          btn.addEventListener("click", () => {
            if (btn.disabled) return;
            openReportIssueModal(btn.getAttribute("data-id"));
          });
        });
        document.querySelectorAll(".reorder-btn").forEach((btn) => {
          btn.addEventListener("click", async () => {
            if (btn.disabled) return;
            btn.disabled = true;
            const orderIdRaw = btn.getAttribute("data-id");
