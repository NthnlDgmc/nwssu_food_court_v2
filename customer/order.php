      ...
      <div class="max-w-5xl mx-auto px-4 pt-3 pb-4 space-y-3">
        <div class="rounded-md bg-white border border-gray-200 p-3 shadow-sm space-y-3">
          ...
          <div
            ...
            class="hidden flex items-center gap-2 overflow-x-auto no-scrollbar"></div>
        </div>
  ...
  <script>
    ...
    function showOrderDetails(orderIdRaw) {
      ...
      const showStallOwner =
        order.stallOwner &&
        (order.orderType === "pickup" || ["pending", "preparing", "cancelled"].includes(order.status));
      const showDeliveryStaff =
        order.deliveryStaff &&
        order.orderType === "delivery" && ["ready_for_dispatch", "collected", "out_for_delivery", "delivered"].includes(
          order.status,
        );
      content.innerHTML = `
          <div>
    ...
    function bindEvents() {
      ...
      document.querySelectorAll(".reorder-btn").forEach((btn) => {
        btn.addEventListener("click", async () => {
          ...
          const res = await postAction("reorder", {
            order_id: orderIdRaw
          });
          if (res.success) {
